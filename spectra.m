...
function spectra(xdata,ydata)
   ...
   %will need to find a way of importing actual spectra
   fig = figure;
   ...
   set(hb,'position',[350,520,200,30],'string','For each peak: Select top, then right, then left.');
   hc = uicontrol('style','togglebutton');
   set(hc,'position',[450,500,100,20],'string','I am finished','callback',@readout);
   hd = uicontrol('style','togglebutton');
